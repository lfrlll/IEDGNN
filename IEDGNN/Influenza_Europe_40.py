...
class DecoupledModel(nn.Module):
    def __init__(self, node_features, out_channels=64, node_emb_dim=64, time_emb_dim=4, hidden_dim=64, lr=0.002,
        ...
        super(DecoupledModel, self).__init__()  # seq_length;node_hidden;node_features不能优化
        ...
        self.frequency_block = FrequencyBlock(k_s, k_t, num_hidden, node_hidden, seq_length, dropout, time_emb_dim,
                                              adjs)
        self.dif_linear = nn.Linear(14, 1)  # in_features必须是num_region
        self.inh_linear = nn.Linear(out_channels, 1)
        self.dif_hid = nn.Linear(14, hidden_dim)  # in_features必须是20
        self.lr = lr
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='Europe', help='Dataset name.')
    ...
    dataloader = load_data(data_dir, batch_size,40)
    ...
    model = DecoupledModel(node_features=3, out_channels=out_channels, node_emb_dim=node_dim,
